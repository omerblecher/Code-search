...
def convert(s, linesize=0, ponct=0):
    ...
    for c in s:
        ...
        # special html chars
            ...
            t += u"&%s;" % (html.entities.codepoint2name[ord(c)])
            i += 1
...
def add_comment(s, output_file):
    output_file.write(('<tr class="diffmisc"><td colspan="4">%s</td></tr>\n'%convert(s)).encode(encoding).decode())


def add_filename(f1, f2, output_file):
    output_file.write(("<tr><th colspan='2'>%s</th>"%convert(f1, linesize=linesize)).encode(encoding).decode())
    output_file.write(("<th colspan='2'>%s</th></tr>\n"%convert(f2, linesize=linesize)).encode(encoding).decode())
...
def add_line(s1, s2, output_file):
    ...
    output_file.write(('<tr class="diff%s">' % type_name).encode(encoding).decode())
    if s1 != None and s1 != "":
        output_file.write(('<td class="diffline">%d </td>' % line1).encode(encoding).decode())
        output_file.write('<td class="diffpresent">'.encode(encoding).decode())
        output_file.write(convert(s1, linesize=linesize, ponct=1).encode(encoding).decode())
    ...
    if s2 != None and s2 != "":
        output_file.write(('<td class="diffline">%d </td>'%line2).encode(encoding).decode())
        output_file.write('<td class="diffpresent">')
        output_file.write(convert(s2, linesize=linesize, ponct=1).encode(encoding).decode())
        output_file.write('</td>')
    else:
        s2 = ""
        output_file.write('<td colspan="2"></td>')
...
def parse_input(input_file, output_file, input_file_name, output_file_name,
    ...
    if not exclude_headers:
        ...
        output_file.write(html_hdr.format(title_suffix, encoding, desc, "", modified_date, lang).encode(encoding).decode())
    output_file.write(table_hdr.encode(encoding).decode())
    ...
    empty_buffer(output_file)
    output_file.write(table_footer.encode(encoding).decode())
    # if not exclude_headers:
        # output_file.write(html_footer.format("", dtnow.strftime("%d.%m.%Y")).encode(encoding).decode())
